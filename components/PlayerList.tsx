          ...
          {isEditing && (
            <div className="w-24">
              ...
              <input
                ...
                onChange={(e) => setFormData(p => ({ ...p, matchPoints: parseInt(e.target.value) || 0 }))}
                className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm"
              />
            </div>
          )}
          ...
          <tbody className="divide-y divide-slate-100">
            {sortedPlayers.map((player, index) => (
              <tr key={player.id} className="hover:bg-slate-50 transition-colors group">
                ...
                <td className="px-6 py-4"><div className="font-bold text-slate-900">{player.name}</div></td>
                ...
                <td className="px-6 py-4 text-center text-xs font-medium text-slate-500">{player.basePoints}</td>
                <td className="px-6 py-4 text-center text-xs font-medium text-slate-500">{player.matchPoints}</td>
                <td className="px-6 py-4 text-center">
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex gap-3 justify-end">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); onDeletePlayer(player.id); }} className="text-slate-300 hover:text-red-600 transition-colors">
