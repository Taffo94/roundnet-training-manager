...
const IconCheck = () => <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>;
      ...
      <div style="font-family: sans-serif; padding: 40px; color: #334155;">
        ...
        ${session.rounds.map(r => `
          <div style="margin-bottom: 30px; page-break-inside: avoid;">
            ...
            ${r.matches.map(m => {
              const s1 = m.status === 'COMPLETED' && m.team1.score !== undefined ? m.team1.score : '';
              const s2 = m.status === 'COMPLETED' && m.team2.score !== undefined ? m.team2.score : '';
                        ...
                        <div className="flex items-center gap-3">
                          {m.status === 'COMPLETED' ? (
                            ...
                            <div className="flex gap-2 bg-slate-50 p-2 rounded-xl border border-slate-200">
                              ...
                              />
                              {isAdmin && (
                                <button onClick={() => { const sc = matchScores[m.id]; if(sc?.s1 && sc?.s2) onUpdateScore(session.id, round.id, m.id, parseInt(sc.s1), parseInt(sc.s2)); }} className="bg-red-600 text-white px-4 h-10 rounded-lg text-[10px] font-black uppercase">
                                  <IconCheck />
                                </button>
                              )}
                            </div>
