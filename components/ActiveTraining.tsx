          ...
          <div key={round.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="p-6 space-y-4">
                    ...
                    {[1, 2].map(t => (
                      <div key={t} className={`space-y-3 ${t === 2 ? 'text-right' : ''}`}>
                        ...
                        {(t === 1 ? m.team1.playerIds : m.team2.playerIds).map((id, idx) => (
                          <div key={idx}>
                            {m.status === 'PENDING' ? (
                              <select value={id} onChange={(e) => onUpdatePlayers(session.id, round.id, m.id, t as 1|2, idx as 0|1, e.target.value)} className="text-[11px] font-bold p-1 bg-white border border-slate-200 rounded outline-none w-full">
                                <option value="">Scegli...</option>
                                {participants.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                              </select>
                            ) : (
                              <button onClick={() => onSelectPlayer(id)} className="text-sm font-black text-slate-800 hover:text-red-600 truncate block w-full text-inherit">{getPlayer(id)?.name || '???'}</button>
                            )}
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="flex items-center gap-2">
                    {m.status === 'COMPLETED' ? (
                      ...
                      <div className="flex gap-2 bg-white p-2 rounded-xl border border-slate-200">
                        ...
                        <button onClick={() => { const sc = matchScores[m.id]; if(sc?.s1 && sc?.s2) onUpdateScore(session.id, round.id, m.id, parseInt(sc.s1), parseInt(sc.s2)); }} className="bg-red-600 text-white px-4 py-2 rounded-lg text-xs font-black uppercase">Ok</button>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {round.restingPlayerIds.length > 0 && (
                <div className="p-4 bg-yellow-50 rounded-xl border border-yellow-100 flex flex-wrap gap-4 items-center">
                  <span className="text-[10px] font-black text-yellow-600 uppercase tracking-widest">In Pausa:</span>
                  {round.restingPlayerIds.map((id, idx) => (
                    <select key={idx} value={id} onChange={(e) => onUpdateResting(session.id, round.id, idx, e.target.value)} className="text-[11px] font-bold p-1 bg-white border border-yellow-200 rounded outline-none">
                      {participants.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                    </select>
                  ))}
                </div>
